    ...
    def end(self, log=False):
        ...
        if log:
            ...
            if not os.path.exists('metrics.csv'):
                ...
                f.write('Test Date and Time,Host PC,Agent Type,Length of Game (sec),Board Size,Number of Moves,Successful Moves,Outcome')
            f = open('metrics.csv', 'a')
            f.write("\n" + test_date_time + ',' + self.host_pc + ',' + self.agent + ',' + game_length + ',' + str(self.board_length) + ',' + str(self.moves) +
                    ',' + str(self.successful_moves) + ','+ self.outcome)
