...
class NeighborReasoningAI():
    ...
    def __init__(self, length):
        ...
        #contains the neighbors of all cells
    ...
    def infer_from_score(self, x, y, n):
        ...
        if n == 0:
            ...
            #initialize neighbor list for cell
            if (x, y) not in self.neighbor_list:
                self.neighbor_list[(x, y)] = set()
                ...
                for j in range(y - 1, y + 2):
                    if 0 < i <= self.board_length and 0 < j <= self.board_length:
                        if not (i == x and j == y):
                            ...
                            self.neighbor_list[(x, y)].add(neighbor)
            ...
            if literal in self.compound_clauses:
                if len(self.compound_clauses[literal]) == 1:
                    for clause in self.compound_clauses[literal]:
                        safe_neighbors = self.neighbor_list[(x, y)]
                        for clause_literal in clause:
                            safe_neighbors.remove(clause_literal)
                            self.add_to_kb(clause_literal[0], clause_literal[1], True)
                        for neighbor in safe_neighbors:
                            self.add_to_kb(neighbor[0], neighbor[1], False)
